...
class WebChangeTarget(BaseModel):
    """A web page to monitor for changes."""
...
class WebChangeConfig(BaseModel):
    """Web page change monitoring configuration."""

    enabled: bool = False
    targets: list[WebChangeTarget] = []


class ProductHuntSourceConfig(BaseModel):
    """Product Hunt tracking configuration."""

    enabled: bool = False
    topics: list[str] = []
    companies: list[str] = []
    fetch_top_daily: int = 20


class GoogleNewsSourceConfig(BaseModel):
    """Google News keyword monitoring configuration."""
...
class CompetitorTrackingSource(BaseModel):
    """Sources to monitor for a single competitor."""
    ...
    web_pages: list[str] = []
    producthunt_slug: str = ""
    news_keywords: list[str] = []
    telegram_channels: list[str] = []


class CompetitorConfig(BaseModel):
    """Configuration for tracking a single competitor."""

    name: str
    slug: str
    enabled: bool = True
    sources: CompetitorTrackingSource = None
    ...
    def __init__(self, **data):
        ...
        if self.sources is None:
            self.sources = CompetitorTrackingSource()
        if not self.signal_types:
            self.signal_types = [s.value for s in SignalType]


class CompetitiveIntelligenceConfig(BaseModel):
    """Competitive intelligence module configuration."""

    enabled: bool = False
    competitors: list[CompetitorConfig] = []
    web_monitoring: WebChangeConfig = None
    producthunt: ProductHuntSourceConfig = None
    googlenews: GoogleNewsSourceConfig = None

    def __init__(self, **data):
        super().__init__(**data)
        if self.web_monitoring is None:
            self.web_monitoring = WebChangeConfig()
        if self.producthunt is None:
            self.producthunt = ProductHuntSourceConfig()
        if self.googlenews is None:
            self.googlenews = GoogleNewsSourceConfig()
...
class SourcesConfig(BaseModel):
    ...
    web_monitoring: WebChangeConfig = Field(default_factory=WebChangeConfig)
    producthunt: ProductHuntSourceConfig = Field(default_factory=ProductHuntSourceConfig)
    googlenews: GoogleNewsSourceConfig = Field(default_factory=GoogleNewsSourceConfig)
...
class Config(BaseModel):
    ...
    competitive_intelligence: Optional[CompetitiveIntelligenceConfig] = None
