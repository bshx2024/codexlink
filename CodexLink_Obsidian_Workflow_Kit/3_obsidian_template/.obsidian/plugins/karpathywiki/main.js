  ...
  async resolvePagePath(name, pageType, summary) {
    ...
    try {
      ...
      const otherMatch = otherTypePages.find((p) => {
        const slugPath2 = `${this.ctx.settings.wikiFolder}/${otherFolder}/${slug}.md`;
        return p.path === slugPath2 || computeSlug(p.title).toLowerCase() === targetSlug || (p.aliases || []).some((a) => computeSlug(a).toLowerCase() === targetSlug);
      });
      if (otherMatch) {
  ...
  async ingestSource(file) {
    ...
    try {
      ...
      if (pageGenRateInfo) {
        ...
        new import_obsidian5.Notice(formatRateLimitNotice(pageGenRateInfo, this.settings.language), 1e4);
      }
      ...
      if (relatedRateInfo) {
        ...
        new import_obsidian5.Notice(formatRateLimitNotice(relatedRateInfo, this.settings.language), 1e4);
      }
      ...
      if (collisions.length > 0) {
        new import_obsidian5.Notice(getText(this.settings.language, "crossTypeCollisionNotice").replace("{count}", String(collisions.length)), 5e3);
      }
      ...
      if (error instanceof DOMException && error.name === "AbortError") {
        ...
        new import_obsidian5.Notice(getText(this.settings.language, "ingestionCancelled"), 5e3);
...
async function runAliasCompletion(ctx, aliasDeficientPages) {
  ...
  if (aliasRateInfo) {
    ...
    new import_obsidian9.Notice(formatRateLimitNotice(aliasRateInfo, ctx.settings.language), 1e4);
  }
  fixNotice.hide();
...
async function runLintWiki(ctx, signal) {
  ...
  Signal: ${c.reason}`
          ...
          if (dedupRateInfo) {
            ...
            new import_obsidian10.Notice(formatRateLimitNotice(dedupRateInfo, ctx.settings.language), 1e4);
          }
...
${results.join("\n")}`);
    ...
    if (error instanceof DOMException && error.name === "AbortError") {
      new import_obsidian10.Notice(getText(ctx.settings.language, "ingestionCancelled"), 5e3);
...
var LLMWikiPlugin = class extends import_obsidian11.Plugin {
  ...
  ingestActiveFile() {
    ...
    if (!activeFile) {
      new import_obsidian11.Notice(getText(this.settings.language, "noActiveFile"), 5e3);
      return;
    }
    if (activeFile.extension !== "md") {
      new import_obsidian11.Notice(getText(this.settings.language, "mdOnlyFile"), 5e3);
      return;
    }
  ...
  requireLLMReady() {
    ...
    new import_obsidian11.Notice(getText(this.settings.language, "llmNotReady"), 8e3);
    return false;
  }
};
